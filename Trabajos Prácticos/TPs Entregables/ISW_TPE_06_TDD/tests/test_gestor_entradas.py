...
def test_compra_si_usuario_no_esta_registrado_falla(gestorDeEntradas):
    ...
    with pytest.raises(PermissionError):
        gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 1, VISITANTE_VALIDO, "Regular", "efectivo", usuario_email="testFalla@email.com")
...
def test_compra_si_cantidad_es_mayor_a_diez_falla(gestorDeEntradas):
    ...
    with pytest.raises(ValueError, match="un número entre 1 y 10"):
        gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 11, [], "VIP", "tarjeta", usuario_email="juan.perez@email.com")
...
def test_compra_si_parque_cerrado_falla(gestorDeEntradas):
    ...
    with pytest.raises(ValueError, match="El parque se encuentra cerrado"):
        gestorDeEntradas.comprar_entradas(FECHA_CERRADA, 1, VISITANTE_VALIDO, "Regular", "tarjeta", usuario_email="juan.perez@email.com")

# Probar comprar entrada con fecha pasada
def test_compra_si_fecha_es_anterior_falla(gestorDeEntradas):
    """Criterio: La fecha de visita no puede ser pasada."""
    with pytest.raises(ValueError, match="La fecha de visita no puede ser en el pasado."):
        gestorDeEntradas.comprar_entradas(AYER, 1, VISITANTE_VALIDO, "Regular", "tarjeta", usuario_email="juan.perez@email.com")
...
def test_compra_sin_forma_de_pago_falla(gestorDeEntradas):
    ...
    with pytest.raises(ValueError, match="Debe seleccionar una forma de pago"):
        gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 1, VISITANTE_VALIDO, "Regular", "", usuario_email="juan.perez@email.com")
...
def test_compra_si_detalles_no_coinciden_con_cantidad_falla(gestorDeEntradas):
    ...
    with pytest.raises(ValueError, match="no coincide con la cantidad"):
        ...
        gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 2, VISITANTE_VALIDO, "Regular", "tarjeta", usuario_email="juan.perez@email.com")
...
def test_compra_si_edad_es_negativa_falla(gestorDeEntradas):
    ...
    with pytest.raises(ValueError, match="La edad de los visitantes debe ser un número positivo."):
        gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 1, visitante_invalido, "Regular", "tarjeta", usuario_email="juan.perez@email.com")
...
def test_compra_si_tipo_pase_es_invalido_falla(gestorDeEntradas):
    ...
    with pytest.raises(ValueError, match="no es válido"):
        gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 1, VISITANTE_VALIDO, "Ultra-VIP-X", "tarjeta", usuario_email="juan.perez@email.com")
...
def test_compra_y_retorna_datos_pasa(gestorDeEntradas):
    ...
    with mock.patch.object(gestorDeEntradas, '_enviar_mail_confirmacion') as mock_email:
        ...
        resultado = gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 2, VISITANTES_VALIDOS, "Regular", "efectivo", usuario_email="juan.perez@email.com")
...
def test_compra_con_tarjeta_simula_redireccion_a_pasarela_pasa(gestorDeEntradas):
    ...
    with mock.patch.object(gestorDeEntradas, '_procesar_pago', return_value=True) as mock_pago:
        ...
        gestorDeEntradas.comprar_entradas(FECHA_FUTURA, 1, VISITANTE_VALIDO, "VIP", "tarjeta", usuario_email="juan.perez@email.com")
