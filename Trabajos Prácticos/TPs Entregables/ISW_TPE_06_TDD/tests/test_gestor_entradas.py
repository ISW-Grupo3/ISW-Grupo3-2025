...
def test_compra_si_usuario_no_esta_registrado_falla(gestor):
...
def test_compra_si_cantidad_es_mayor_a_diez_falla(gestor):
...
def test_compra_si_parque_cerrado_falla(gestor):
...
def test_compra_si_fecha_es_pasada_falla(gestor):
...
def test_compra_sin_forma_de_pago_falla(gestor):
...
def test_compra_si_detalles_no_coinciden_con_cantidad_falla(gestor):
...
def test_compra_si_edad_es_negativa_falla(gestor):
...
def test_compra_si_tipo_pase_es_invalido_falla(gestor):
...
# --- TESTS DE CA (PASAN) ---
def test_compra_y_retorna_datos_pasa(gestor):
...
def test_compra_con_tarjeta_simula_redireccion_a_pasarela_pasa(gestor):
