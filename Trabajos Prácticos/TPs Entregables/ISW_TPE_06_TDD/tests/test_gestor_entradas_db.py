...
@pytest.fixture
def gestorDeEntradas(db_test):
    """Crea un gestorDeEntradas con base de datos de prueba"""
    return GestorDeEntradas("test_parque.db")

# --- TESTS DE CA (FALLAN) ---

def test_compra_falla_si_usuario_no_esta_registrado(gestorDeEntradas):
    """Criterio: Solo usuarios registrados pueden comprar."""
    with pytest.raises(PermissionError, match="usuarios registrados"):
        gestorDeEntradas.comprar_entradas(
            FECHA_FUTURA, 1, VISITANTE_VALIDO, "Regular", "efectivo", 
            EMAIL_USUARIO_INVALIDO
        )

def test_compra_falla_si_cantidad_es_mayor_a_diez(gestorDeEntradas):
...
def test_compra_falla_si_parque_cerrado(gestorDeEntradas):
...
def test_compra_falla_si_fecha_es_pasada(gestorDeEntradas):
...
def test_compra_falla_sin_forma_de_pago(gestorDeEntradas):
...
def test_compra_falla_si_detalles_no_coinciden_con_cantidad(gestorDeEntradas):
...
def test_compra_falla_si_edad_es_negativa(gestorDeEntradas):
...
def test_compra_falla_si_tipo_pase_es_invalido(gestorDeEntradas):
...
def test_compra_falla_si_excede_capacidad(gestorDeEntradas):
...
def test_compra_exitosa_y_retorna_datos(gestorDeEntradas):
    """Prueba de usuario: Compra exitosa (pasa)."""
    
    resultado = gestorDeEntradas.comprar_entradas(
    ...
    compra = gestorDeEntradas.obtener_compra(resultado["compra_id"])
...
def test_compra_con_tarjeta_simula_redireccion_a_pasarela(gestorDeEntradas):
...
def test_confirmacion_email_se_envia(gestorDeEntradas):
...
def test_registro_de_nuevo_usuario(gestorDeEntradas):
    """Test adicional: Registro de nuevos usuarios."""
    nuevo_email = "nuevo@example.com"
    usuario_id = gestorDeEntradas.registrar_usuario(nuevo_email, "Usuario Nuevo")
    
    assert usuario_id > 0
    assert gestorDeEntradas.db.usuario_registrado(nuevo_email)

def test_consulta_disponibilidad(gestorDeEntradas):
    """Test adicional: Consultar disponibilidad de entradas."""
    # Al inicio debería haber capacidad máxima disponible
    disponibles = gestorDeEntradas.obtener_disponibilidad(FECHA_FUTURA)
...
def test_flujo_completo_compra_exitosa(gestorDeEntradas):
    """Test de integración: Flujo completo de compra exitosa."""
    
    # 1. Verificar que el usuario está registrado
    assert gestorDeEntradas.db.usuario_registrado(EMAIL_USUARIO_VALIDO)
    ...
    compra = gestorDeEntradas.obtener_compra(compra_id)
